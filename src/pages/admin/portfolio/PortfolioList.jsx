...
import { Plus, Search, Edit2, Trash2, Eye, EyeOff, GripVertical, Image as ImageIcon, RefreshCcw } from 'lucide-react';
import { supabase } from '../../../lib/supabase';
import '../../../components/admin/AdminLayout.css';
import FacebookPortfolioManager from './FacebookPortfolioManager';

export default function PortfolioList() {
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState('manual'); // 'manual' or 'facebook'
  ...
  useEffect(() => { 
      if (activeTab === 'manual') fetchItems(); 
  }, [activeTab]);
    ...
    <div>
      <div className="admin-page-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          ...
          <p className="admin-page-subtitle">Kelola foto produk & hasil sinkronisasi Facebook</p>
        </div>
        {activeTab === 'manual' && (
      ...
      <div style={{ display: 'flex', borderBottom: '1px solid #e2e8f0', marginBottom: '1.5rem' }}>
        <button 
            ...
            style={{
                ...
                borderBottom: activeTab === 'manual' ? '2px solid #0d9488' : '2px solid transparent',
                color: activeTab === 'manual' ? '#0d9488' : '#64748b'
            }}
        >
            <ImageIcon size={16} /> Portfolio Web (Manual)
        </button>
        <button 
            ...
            style={{
                background: 'none', border: 'none', padding: '0.75rem 1.5rem', cursor: 'pointer',
                display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.875rem', fontWeight: '600',
                borderBottom: activeTab === 'facebook' ? '2px solid #3b5998' : '2px solid transparent',
                color: activeTab === 'facebook' ? '#3b5998' : '#64748b'
            }}
        >
            <RefreshCcw size={16} /> Sinkronisasi Facebook
        </button>
      </div>
      ...
      {/* Facebook Sync Content */}
      {activeTab === 'facebook' && (
          <FacebookPortfolioManager />
      )}
    </div>
  );
}
